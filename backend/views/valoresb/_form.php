...
<table style="width:100%;">
   ...
   ?>
    ...
    <tr>
        <td><?= $form->field($valoresb, "[$index]ingenieria")->hiddenInput(['value'=>$ingenierias[$index]])->label(false)?><label><?php echo $ingenierias[$index]?></label></td>
        <td><?= $form->field($valoresb, "[$index]Total")->textInput(['maxlength' => true])->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]hombres")->textInput(['maxlength' => true])->label(FALSE) ?></td> 
        <td><?= $form->field($valoresb, "[$index]mujeres")->textInput(['maxlength' => true])->label(FALSE) ?></td>       
        <td><?= $form->field($valoresb, "[$index]pertenecenEtnia")->textInput(['maxlength' => true])->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]HabitanEnElEstado")->textInput()->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]DiscapacidadMotriz")->textInput(['maxlength' => true])->label(FALSE) ?></td>  
        <td><?= $form->field($valoresb, "[$index]DiscapacidadMental")->textInput(['maxlength' => true])->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]DiscapacidadSensorial")->textInput()->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]DiscapacidadPsicosocial")->textInput(['maxlength' => true])->label(FALSE) ?></td>
        <td><?= $form->field($valoresb, "[$index]DiscapacidadComunicacion")->textInput(['maxlength' => true])->label(FALSE) ?></td>
   ...
   <?php } ?>
    ...
    <div class="form-group">
        <?= Html::submitButton($valoresb->isNewRecord ? 'Guardar' : 'Update', ['class' => $valoresb->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
    <?php ActiveForm::end(); ?>
